...
%3D_Landmarc_Localization_Algorithm_Parametric 
%-------------------------------------------------------------------------
tic
fl=1;
f2=1;
...
while k_neighbors<9
    dt=1;%(sec)
    while dt<3
        common_samples_threshold=5;
        while common_samples_threshold<9
            %Read tables 
            ...
            Reader_table = readtable('ola_mesa.txt','ReadVariableNames',true,'Delimiter','comma');
            ...
            while i<m
                ...
                nrowC=size(C,1);
                for k=1:nrowC
                    % I have to update the number of occurences of each tag right here(how many times it has been read in this experiment)
                    index_epc1=find(contains(List_of_all_tags,C{k,1}));
                    Number_of_occurences(index_epc1)=Number_of_occurences(index_epc1)+1;
                    for j=k+1:nrowC
            ...
            for k=49:length(Estimations_RSSI.EPC)
                if Estimations_RSSI.Errors(k)>0
                    s1=s1+sqrt((Estimations_RSSI.Errors(k)-mesosfalma)^2);
                    y1=y1+1;
                end 
            end
            std=sqrt(s1/y1);
            fprintf('k : %d ||  dt : %d ||  C : %d ||  MeanError : %f || Std: %f  || Tags Found : %d \n',k_neighbors,dt,common_samples_threshold,mesosfalma,std,num);
            fileID3 = fopen('param_new_results.txt','a');
            if fl==1
                fprintf(fileID3,'k\t dt\t \t C\t MeanError\t Num\t Std\t  \n');
                fl=0;
            end
            fprintf(fileID3,'%d\t %d\t %d\t %f\t %d\t %f\n',k_neighbors,dt,common_samples_threshold,mesosfalma,num,std);
            fclose(fileID3);
            common_samples_threshold=common_samples_threshold+1;
        end
        dt=dt+1;
    end
